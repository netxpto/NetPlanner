...
X represents the number of possible of ligt paths that can be established (OMS) between each pair of nodes. Initially there is none but as demands are being processed and we start to build our network OMS will be established and used to route those demands.\\
...
\end{table}
This signal is formed by one boolean variable "routed"  which will return true in the case a demand is routed correctly through the network, validating the remaining information, or false in the case it is not which means no path was created to route the demand and so the other fields of this structure will be void or filled with invalid information. Also this signal has contains a structure to represent the light paths needed to established the path required to route the demand being processed, if it possible to do so. A PathRequestRouted signal can contain one or many lightPaths if more than one is needed to create the path requested.
...
\begin{tabular}{|c|c|c|c|c|c|}
	...
	\multirow{4}{*}{0...D-1} & \multirow{4}{*}{\begin{tabular}[c]{@{}c@{}}true/\\ false\end{tabular}} & \multirow{4}{*}{0...P-1} & \multirow{2}{*}{1...L-1} & 1...TOC-1           & \multirow{2}{*}{1...OC} \\ \cline{5-5}
	&                                                                        &                          &                          & ...                 &                         \\ \cline{4-6} 
...
\begin{table}[H]
	...
	\begin{tabular}{|c|c|c|}
		...
		\begin{tabular}[c]{@{}c@{}}LogicalTopologyManager\_  \end{tabular} & \begin{tabular}[c]{@{}c@{}}criterion, numberOfPaths\end{tabular} & \begin{tabular}[c]{@{}c@{}} SchedulerOut, \\LogicalTopologyOut, \\PhysicalManagerOut \end{tabular}                                                                        \\ \hline
		PhysicalTopologyManager\_                                                          & \begin{tabular}[c]{@{}c@{}}adjacenceMatrix, transportSystems, \\opticalChannels, \\opticalChannelCapacity\end{tabular}                                                                                                & LogicalManagerRequest      \\ \hline
