...
\textbf{A}: An ordered queue that contains all the demands in an order defined previously by the
...
\textbf{B}: A ordered queue with the k shortest Optical paths for the demand being processed atthat time.

\textit{Optical Path} \\
Index\\
Links Used: Array of integers with 16 slots, whose value will be 1/0 depending on if they are being used or not.\\

\textbf{C}: Creation of the network links with a certain number of optical channels and capacity per channel.
...
Source node\\
Destination node\\
Boolean blocking = false; (true == if link can´t route a demand)\\

\textbf{D}: Here we have the path that at a certain time is on top of the queue and so it will be thefirst to be tried.\\

\textbf{E}: Signal that contains demands blocked in our network.\\

\textbf{F}: The demand is associated to a path and a route is created.\\
...
\textit{Demand-Path-Route}\\

Demand Index: integer\\
Path Index: integer\\
Route Index:\\

\textbf{G}: Signal that contains demands routed in our network.
